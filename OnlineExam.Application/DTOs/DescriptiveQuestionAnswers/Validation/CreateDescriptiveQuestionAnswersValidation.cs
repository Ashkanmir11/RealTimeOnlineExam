...
{
    public class CreateDescriptiveQuestionAnswersValidation : AbstractValidator<CreateDescriptiveQuestionAnswersDTO>
    {
        private readonly IAccountRepository _accountRepository;
        private readonly IDescriptiveQuestionRepository _descriptiveQuestionRepository;

        public CreateDescriptiveQuestionAnswersValidation(IAccountRepository accountRepository, IDescriptiveQuestionRepository descriptiveQuestionRepository)
        {
            ...
            RuleFor(e => e.descriptiveQuestionAnswersId).MustAsync(async (Id, Token) =>
            {
                return await _descriptiveQuestionRepository.ExistAsync(Id);
            }).WithMessage((Model) => $"سوالی با آیدی {Model.descriptiveQuestionAnswersId} یافت نشد.");
        }
    }
}
